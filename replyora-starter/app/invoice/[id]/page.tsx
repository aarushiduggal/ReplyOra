      ...
      <div className="mx-auto max-w-2xl px-8 py-10">
        <div className="mb-8 flex items-start justify-between">
          <div>
            ...
            {billing.businessEmail && <p className="text-[12px] text-ink/85">{billing.businessEmail}</p>}
          </div>
          <div className="text-right">
            ...
            <p className="text-[12px] text-ink/85">Issued {invoice.issuedAt?.slice(0, 10) ?? "—"}</p>
            <p className="text-[12px] text-ink/85">Due {invoice.dueAt?.slice(0, 10) ?? "—"}</p>
          </div>
        </div>

        <div className="mb-6">
          <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-ink/80">Bill to</p>
          <p className="mt-1 text-sm font-semibold text-ink">{invoice.billTo?.name || client?.name || "Client"}</p>
          {invoice.billTo?.email && <p className="text-[12px] text-ink/85">{invoice.billTo.email}</p>}
        </div>

        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="border-b border-ink/20 text-left text-[10px] font-semibold uppercase tracking-[0.14em] text-ink/80">
          ...
          <tbody>
            {invoice.lineItems.map((it, i) => (
              <tr key={i} className="border-b border-ink/10">
                ...
                <td className="py-2.5 text-center text-ink/85">{it.quantity}</td>
                <td className="py-2.5 text-right text-ink/85">{money(it.unitCents, invoice.currency)}</td>
        ...
        <div className="mt-4 ml-auto w-56 space-y-1 text-sm">
          <div className="flex justify-between text-ink/85"><span>Subtotal</span><span>{money(sub, invoice.currency)}</span></div>
          <div className="flex justify-between text-ink/85"><span>Tax ({invoice.taxRate}%)</span><span>{money(taxCents, invoice.currency)}</span></div>
        ...
        {billing.terms && (
          <p className="mt-8 text-[11px] text-ink/80">{billing.terms}</p>
        )}
